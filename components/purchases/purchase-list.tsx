...
export function PurchaseList({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border">
      {/* Header */}
      <div className="grid grid-cols-[4rem_1fr_1fr_5rem_2rem] items-center gap-2 border-b border-border bg-card px-4 py-2.5 text-xs font-medium text-muted">
        <span>Date</span>
      ...
      {purchases.map((p) => (
        ...
        >
          <div>
            ...
            {p.notes && (
              <p className="mt-1 truncate text-xs text-muted/50" title={p.notes}>
                {p.notes}
              </p>
            )}
          </div>
